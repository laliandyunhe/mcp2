 ...
 *
 * 说明:
 * - Roche 插件官方 API 里没有"直接把消息塞进对话"的接口,唯一正式的
 *   写回渠道是 roche.memory.write() 写入主事实记忆。所以工具结果不会
 *   实时出现在这条消息下面,而是作为记忆,在 AI 下一次生成回复时被读到。
...
(function () {
  ...
  async function checkConversation(roche, convId) {
    ...
    for (const msg of msgs) {
      ...
      // 只处理 AI/assistant 消息,不处理用户自己发的
      const isAssistant = msg.type === "assistant" || msg.role === "assistant" || msg.role === "ai";
      if (!isAssistant) continue;

      const calls = parseToolCalls(msg.text || msg.content || "");
