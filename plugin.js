/**
 * MCP 工具桥接插件(最终版 - 强制刷新版)
 *
 * 新增特性：
 * 1. 强行 DOM 注入：检测到工具执行后，强行在当前聊天屏幕画出一个气泡，无需退回列表。
 * 2. 开机自启：利用 window.Roche 全局对象，无需打开插件面板，后台自动开启监控。
 */
(function () {
  "use strict";
  ...
  async function testAndListTools(server) {
    if (server.transport === "sse") {
      throw new Error("调试面板暂时只支持直接测试 Streamable HTTP");
    }
  ...
  // ============================================================
  // ⚡ 核心黑科技 1：强行向当前屏幕注入一个聊天气泡
  // ============================================================
  function forceInjectUIMessage(toolName, result) {
    try {
      // 找屏幕上最大的可以滚动的框（99%是聊天框）
      const divs = Array.from(document.querySelectorAll('div'));
      let chatContainer = divs.find(el => {
        const style = window.getComputedStyle(el);
        return (style.overflowY === 'auto' || style.overflowY === 'scroll') 
            && el.scrollHeight > el.clientHeight
            && el.clientHeight > 200; // 高度要足够大
      });
      ...
      `;
      // 防止内容过长把屏幕撑爆
      const resultStr = JSON.stringify(result).length > 500 
        ? JSON.stringify(result).slice(0, 500) + '... (内容过长已折叠)' 
        : JSON.stringify(result);
        
      bubble.innerHTML = `<strong>🔧 工具 [${toolName}] 执行成功</strong><hr style="border:0; border-top:1px solid #3b6ef0; margin: 8px 0;"/><span style="font-size:12px; color:#ccc;">${resultStr}</span>`;

      if (chatContainer) {
        // 如果找到了内层容器，插进去
        const target = chatContainer.lastElementChild && chatContainer.lastElementChild.tagName === 'DIV' 
        ...
        if (window.Roche && window.Roche.ui) {
            window.Roche.ui.toast(`🔧 工具 [${toolName}] 执行完毕！`);
        }
      }
    } catch (e) {
  ...
  async function writeToolResult(roche, convId, call, result) {
    ...
    try {
      ...
      // ✅ 强制触发 UI 气泡注入，不用切出去了！
      forceInjectUIMessage(call.name, result);
  ...
  function parseToolCalls(text) {
    ...
    while ((m = re.exec(text)) !== null) {
      let args = {};
      try { args = JSON.parse(m[2]); } catch (e) { continue; }
  ...
  async function checkConversation(roche, convId) {
    ...
    let conv = null;
    try { conv = await roche.conversation.get(convId); } catch (e) {}
        ...
        async mount(container, roche) {
          ...
          function openEdit(server) {
            ...
            if (state.editing.cachedTools.length > 0) {
              ...
              state.testMessage = `上次测试成功,缓存了 ${state.editing.cachedTools.length} 个工具`;
            } else {
  ...
  // ============================================================
  // ⚡ 核心黑科技 2：插件随 Roche 软件启动时，自动在后台拉起监控
  // ============================================================
  function initAutoStart() {
    let retryCount = 0;
    // 每秒检查一次，直到发现 Roche 本体加载完毕
    const timer = setInterval(async () => {
      retryCount++;
      if (retryCount > 30) clearInterval(timer); // 超过 30 秒如果还没加载就不等了

      // 检查全局对象 window.Roche 是否准备就绪
      if (window.Roche && window.Roche.storage && window.Roche.memory) {
        clearInterval(timer);
        
        GLOBAL_STATE.roche = window.Roche;
        GLOBAL_STATE.servers = await loadServers(window.Roche);
        GLOBAL_STATE.monitorConvIds = await loadMonitorConvIds(window.Roche);
        GLOBAL_STATE.lastProcessedTs = await loadLastProcessedTs(window.Roche);

        const shouldResume = await loadMonitorEnabled(window.Roche);
        
        // 如果你上次打开了监控开关，并且勾选了会话，它就会在后台自动开启！
        if (shouldResume && GLOBAL_STATE.monitorConvIds.length > 0 && !GLOBAL_STATE.isMonitoring) {
          startMonitor();
          // 如果系统提示 UI 也加载完了，可以弹个提示告诉你自启成功了
          if (window.Roche.ui) {
            window.Roche.ui.toast("⚡ MCP 工具桥接：检测到应用启动，已在后台自动恢复监控");
          }
        }
      }
    }, 1000);
