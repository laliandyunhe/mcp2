...
// ============================================================
// ⚡ 核心黑科技：强行向当前屏幕注入一个聊天气泡/弹窗
// toolName: 工具名称
// content: 执行结果或报错信息
// isError: 是否是执行失败
// ============================================================
function forceInjectUIMessage(toolName, content, isError = false) {
  try {
    // 1. 大海捞针：在当前网页找所有的 div，找出那个又高又大还能滚动的（99%概率是当前的聊天记录容器）
    const divs = Array.from(document.querySelectorAll('div'));
    let chatContainer = divs.find(el => {
      const style = window.getComputedStyle(el);
      return (style.overflowY === 'auto' || style.overflowY === 'scroll') 
          && el.scrollHeight > el.clientHeight
          && el.clientHeight > 200; // 高度大于200px
    });

    // 2. 准备画一个气泡
    const bubble = document.createElement("div");
    
    // 根据是成功还是失败，设置不同的颜色（成功蓝色，失败红色）
    const bgColor = isError ? '#4a1a1a' : '#2a3a6e';
    const borderColor = isError ? '#f87171' : '#3b6ef0';
    const textColor = isError ? '#fca5a5' : '#9ec2ff';
    const titleText = isError ? `❌ 工具 [${toolName}] 执行失败` : `✅ 工具 [${toolName}] 执行成功`;
    ...
    // 3. 把过长的内容截断，防止塞爆屏幕
    let contentStr = typeof content === 'object' ? JSON.stringify(content) : String(content);
    if (contentStr.length > 500) {
      contentStr = contentStr.slice(0, 500) + '... (内容过长已折叠)';
    }
      
    // 把文字塞进气泡里
    bubble.innerHTML = `<strong>${titleText}</strong><hr style="border:0; border-top:1px solid ${borderColor}; margin: 8px 0;"/><span style="font-size:12px; color:#ccc;">${contentStr}</span>`;
    ...
    if (chatContainer) {
      ...
      // 万一用户当前没在聊天界面，退而求其次，弹一个 Roche 原生的全局 Toast 提示
      if (window.Roche && window.Roche.ui) {
          window.Roche.ui.toast(`${titleText}！退回重进对话可查看详细。`);
      }
    }
  } catch (e) {
    console.error("注入气泡失败：" + e.message);
  }
}
    ...
    for (const msg of msgs) {
      ...
      for (const call of calls) {
        ...
        try {
          ...
          forceInjectUIMessage(call.name, result, false);
        } catch (e) {
          log(`❌ ${call.name} 调用失败: ${e.message}`);
          forceInjectUIMessage(call.name, e.message, true);
        }
      }
    }
